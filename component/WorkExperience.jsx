...
const WorkExperience = ({ onChange = () => {}, defaultValue = [], readOnly = false }) => {
  const [experiences, setExperiences] = useState(
    defaultValue.length > 0
      ? defaultValue
      : [
          { jobTitle: '', companyName: '', startDate: '', endDate: '', salary: '', reasonForLeaving: '', supervisorContact: '' },
          { jobTitle: '', companyName: '', startDate: '', endDate: '', salary: '', reasonForLeaving: '', supervisorContact: '' },
          { jobTitle: '', companyName: '', startDate: '', endDate: '', salary: '', reasonForLeaving: '', supervisorContact: '' },
          { jobTitle: '', companyName: '', startDate: '', endDate: '', salary: '', reasonForLeaving: '', supervisorContact: '' },
        ]
        ...
        <table className="min-w-full border text-[12px]">
          <thead>
            <tr className="text-gray-700">
              {headers.map((head, i) => (
                <th key={i} className="border border-gray-300 px-1">{head.label}</th>
              ))}
            </tr>
          </thead>
          <tbody>
                ...
                {headers.map((col, colIndex) => (
                  <td key={colIndex} className="border border-gray-300 px-1">
                    <input
                      ...
                      value={exp[col.key]} 
                      onChange={(e) =>
                        handleExperienceChange(rowIndex, col.key, e.target.value)
                      }

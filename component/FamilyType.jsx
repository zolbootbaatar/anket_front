...
const FamilyType = ({ onChange = () => {}, defaultValue = {}, readOnly = false }) => {
  const [familyCount, setFamilyCount] = useState(defaultValue.familyCount || "");
  ...
  useEffect(() => {
    if (defaultValue.familyCount !== undefined) {
      setFamilyCount(defaultValue.familyCount);
    }
    if (defaultValue.members && defaultValue.members.length > 0) {
    ...
    <div className="relative p-6 border border-gray-400">
      ...
      <div className="mt-2">
        ...
        <label className="text-[12px] ml-2">
          (Аав, ээж, төрсөн ах, эгч, дүү, эхнэр хүүхдийн мэдээллийг заавал оруулах)
        </label>
      </div>
        ...
        <table className="min-w-full border text-[12px]">
          ...
          <tbody>
            {members.map((member, i) => (
              <tr key={i}>
                {["relation", "fullName", "age", "registrationNumber", "currentJob", "contactPhone"].map((field) => (
                  <td className="border px-1" key={field}>
                    <input
                      type={field === "age" ? "number" : "text"}
                      value={member[field] || ""}
                      onChange={(e) => handleMemberChange(i, field, e.target.value)}
